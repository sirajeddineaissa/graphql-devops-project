...
const OrderDetails = () => {
  ...
  return (
    <DetailsOrdered>
        ...
        {cart.length > 0 ? (
          cart.map((product) => (
            <OrderDetailsMiddleItem key={product.cartId}>
              ...
              <span>
                {CURRENCY + Math.round(getCartProductTotalPrice(cart, product))}
              </span>
            </OrderDetailsMiddleItem>
          ))
        ) : (
      ...
      <PaymentMethods>
        <PaymentMethodItem>
          <Radio name="payment" id="dbt" label="Direct Bank Transfer" />
        </PaymentMethodItem>

        <PaymentMethodItem>
          <Radio name="payment" id="cp" label="Check Payments" />
        </PaymentMethodItem>

        <PaymentMethodItem>
          <Radio name="payment" id="cod" label="Cash on Delivery" />
        </PaymentMethodItem>
      </PaymentMethods>
